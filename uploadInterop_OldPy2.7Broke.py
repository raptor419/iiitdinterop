...
def images_for(path):
    ...
    for match in glob.glob("%s/*" % path):
        # print match
        if match.lower()[-4:] == ".jpg" and match.lower()[34]=="i":
            i.append(match)
...
class App():
    ...
    def crop_image(self):
        ...
        input_loc = splPath[34:]
        output_loc = splPath[34:]
    ...
    def gps(self):
        ...
        metadata.read()
        lat=metadata['Exif.Image.Make']
        x.append(lat.value)
        lon=metadata['Exif.Image.Model']
        x.append(lon.value)
    ...
    def fit_to_box(self,label):
        ...
        label.place(x=self.box_width/2, y=self.box_height/2, anchor=tk.CENTER)
